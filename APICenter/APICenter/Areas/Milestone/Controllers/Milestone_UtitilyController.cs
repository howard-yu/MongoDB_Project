...
namespace APICenter.Areas.Milestone.Controllers
{
    public class Milestone_UtitilyController : ApiController
    {
        ...
        int callCnt;
        [HttpPost]
        public IHttpActionResult GetMileStoneAPI([FromBody]List<SaveMilestone> mdList, string userID, string requestType, string apiKey, string Url)
        {
            string responseStr = "";
            try
            {
                var jsonText = Newtonsoft.Json.JsonConvert.SerializeObject(mdList);
                var jsonBytes = Encoding.UTF8.GetBytes(jsonText);
                string datatype = "2";
                string charset = "UTF-8";
                string jsonStr = jsonText + apiKey;
                string datasign = MD5Encrypt64(jsonStr, charset);
                string apiURL = $"{Url}?userID={userID}&requestType={requestType}&datatype={datatype}&charset={charset}&APIKey={apiKey}&datasign={datasign}";
            ...
            return Ok(new { errorCode = 0, IsSuccess = true, responseMsg = responseStr });
        }

        public string MD5Encrypt64(string encryptionString, string charset)
        {
            //string pwd = "";
            byte[] bytes = System.Text.Encoding.GetEncoding(charset).GetBytes(encryptionString);
