//------------------------------------------------------------------------------
// <auto-generated>
//     這個程式碼是由範本產生。
//
//     對這個檔案進行手動變更可能導致您的應用程式產生未預期的行為。
//     如果重新產生程式碼，將會覆寫對這個檔案的手動變更。
// </auto-generated>
//------------------------------------------------------------------------------

namespace MongoAppdemo
{
    using System;
    using System.Collections.Generic;
    
    public partial class OEISF5CNTR
    {
        public int ID { get; set; }
        public Nullable<int> ISF5ID { get; set; }
        public string CNTR_DESC { get; set; }
        public string CNTR_INIT { get; set; }
        public string CNTR_NMBR { get; set; }
        public string CNTR_CHECK_DIGIT { get; set; }
        public string CNTR_SIZE_TYPE { get; set; }
        public string CRT_USR { get; set; }
        public Nullable<System.DateTime> CRT_DTM { get; set; }
        public string UPD_USR { get; set; }
        public Nullable<System.DateTime> UPD_DTM { get; set; }
    }
}
